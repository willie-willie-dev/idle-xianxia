...
> ⚠️ **不一致**：`04-事件系统.md` 描述 REALM_ORDER 包含炼气→筑基→金丹→元婴→化神→合体→大乘→渡劫→仙人（共9个），但 `realms.ts` 只有5个。事件中 `realmRange` 使用的是短列表。
...
**文档 `04-事件系统.md` 描述的 6 条事件**（`cave-treasure`、`secret-realm`、`ancient-ruin`、`heavenly-tribulation`、`dao-test`）**代码中不存在**。
